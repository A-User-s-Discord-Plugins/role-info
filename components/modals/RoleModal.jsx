...
import { Modal, Flex, TabBar, Clickable, Tooltip, Icon } from "@vizality/components";
...
let Role = memo(({ children, color }) => {
	return <>
		<div
			className={`role-2irmRk flex-1O1GKY alignCenter-1dQNNs ${Classes.normalColor} ${Classes.role}`}
			style={{ borderColor: color }}
		>
			<div
				className="roleCircle-3xAZ1j flex-1O1GKY alignCenter-1dQNNs justifyCenter-3D2jYp"
            ...
            <Modal size={Modal.Sizes.MEDIUM}>
                <div className={Classes.topSectionNormal}>
                    ...
                    <div className={Classes.tabBarContainer}>
                        ...
                        >
                            <TabBar.Item className={Classes.tabBarItem} id="Overall">
                                Overall
                            </TabBar.Item>
                            <TabBar.Item className={Classes.tabBarItem} id="Permissions">
                                Permissions
                            </TabBar.Item>
                        </TabBar>
                    </div>
...
const renderTab = function (role, tabname) {
    switch (tabname) {
        case "Overall":
            return <OverallLook role={role}></OverallLook>;
        case "Permissions":
            return <Permissions role={role}></Permissions>;
...
const getIconFromFeatures = function (features) {
    return (
        <Flex className={Classes.profileBadges + " roleinfo-badges"}>
            {Object.entries(features).map((key) => {
				return <Tooltip
					className={Classes.profileBadgeWrapper}
					position="top"
					text={(key[0][0].toUpperCase() + key[0].slice(1))
						.replace(/([a-z])([A-Z])/g, "$1 $2")
						.trim()}
				>
					<Clickable role="button" tag="div">
						{key[0] === "boost" && key[1] && (
							<Icon name="PremiumGuildTier3" />
						)}
						{key[0] === "administrator" && key[1] && (
							<MaterialIcons name="account_circle" />
						)}
						{key[0] === "ban" && key[1] && (
							<MaterialIcons name="hardware" />
						)}
						{key[0] === "kick" && key[1] && (
							<MaterialIcons name="remove_circle_outline" />
						)}
						{key[0] === "mentionEveryone" && key[1] && (
							<MaterialIcons name="alternate_email" />
						)}
						{key[0] === "manageMessages" && key[1] && (
							<MaterialIcons name="chat_bubble" />
						)}
						{key[0] === "manageGuild" && key[1] && (
							<MaterialIcons name="dns" />
						)}
						{key[0] === "manageChannels" && key[1] && (
